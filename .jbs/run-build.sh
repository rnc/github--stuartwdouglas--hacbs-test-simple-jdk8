...
if [ -n "" ]; then
  echo "Setting version to 1.2.4 to match enforced version"
  mvn -B -e -s "/var/workdir/software/settings/settings.xml" -t "/var/workdir/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="1.2.4" | tee /var/workdir/workspace/logs/enforce-version.log
fi
