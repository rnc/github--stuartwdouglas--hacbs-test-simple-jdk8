...
if [ -n "" ]; then
  ...
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="1.2.4" | tee /root/project/logs/enforce-version.log
fi
